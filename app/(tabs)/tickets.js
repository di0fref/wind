...
export default function Tickets() {
    ...
    const filterTickets = () => {
        if (mineOnly) {
            const t = tickets?.filter(ticket => ticket.user_id === user.id)
            setFilteredTickets(t)
        } else {
        ...
        return (
            <View className={"h-full flex-1 bg-neutral-100"}>
                <MainView onRefresh={onRefresh} bg={"#eee"}>
                    ...
                    <OnlyMineCheckBox val={mineOnly} setVal={setMineOnly}/>
                    {/*<NewTickerButton/>*/}
                    {filteredTickets?.map(ticket => <Ticket key={ticket.id} data={ticket}/>)}
